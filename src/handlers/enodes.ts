...
import Store, { isENode } from '#app/store';
...
function parsePayload(payload: string): string[] {
  try {
    const enodes = JSON.parse(payload) as unknown[];
    if (!Array.isArray(enodes)) return [];
    return enodes.filter((enode: unknown) => isENode(enode));
  } catch (error) {
    return [];
  }
}
...
export default async (req: http.IncomingMessage, res: http.ServerResponse) => {
  if (req.method === 'POST') {
    ...
    Store.addENodes(parsePayload(Buffer.concat(buffers).toString()));
  ...
  if (req.method === 'GET' && typeof req.url === 'string') {
    try {
      ...
      if (isValidParams(query)) {
        res.setHeader('Content-Type', 'application/json');
        res.writeHead(200).end(JSON.stringify(await Store.getENodes(query.count)));
        return;
      }
    } catch (err) {
